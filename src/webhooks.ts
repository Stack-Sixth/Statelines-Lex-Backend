...
export function validateEndpoint(value: string, allowedHosts: string[]): URL {
  ...
  if (
    ...
    throw new AppError(
      422,
      'invalid_webhook_url',
      'Use an HTTPS URL on an explicitly allowed hostname, port 443',
    );
  return url;
}
...
export function publicAddress(address: string) {
  if (isIP(address) !== 4) return false; // Deliberately IPv4-only until IPv6 range validation is added.
...
export function webhookSender(config: Pick<Config, 'allowedHosts'>): Sender {
      ...
      return await new Promise<SendResult>((resolve) => {
        ...
        const req = https.request(
          ...
          (res) => {
            ...
            res.on('error', () =>
              finish({ ok: false, retryable: true, status: 0, error: 'Response stream failed' }),
            );
            res.on('end', () => {
        ...
        req.on('error', () =>
          finish({ ok: false, retryable: true, status: 0, error: 'Webhook connection failed' }),
        );
        req.end(body);
      });
    } catch (error) {
