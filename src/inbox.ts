...
export async function acceptEvent(
  ...
  let envelope: z.infer<typeof eventSchema>;
  try {
  ...
  return db.transaction(async (sql) => {
    const inserted = await sql.query(
      'INSERT INTO lex_receiver.inbox(source,event_id,envelope) VALUES($1,$2,$3) ON CONFLICT(source,event_id) DO NOTHING RETURNING event_id',
      [envelope.source, envelope.event_id, JSON.stringify(envelope)],
    );
    const saved = (
      await sql.query<{ state: string; envelope: unknown }>(
        'SELECT state,envelope FROM lex_receiver.inbox WHERE source=$1 AND event_id=$2',
