...
try {
  await db.transaction(async (sql) => {
    ...
    await sql.query(
      'CREATE SCHEMA IF NOT EXISTS lex; CREATE TABLE IF NOT EXISTS lex.schema_migrations(name text PRIMARY KEY,checksum text NOT NULL,applied_at timestamptz NOT NULL DEFAULT now())',
    );
    for (const name of (await readdir('migrations')).filter((n) => n.endsWith('.sql')).sort()) {
      ...
      const row = (
        await sql.query<{ checksum: string }>(
          'SELECT checksum FROM lex.schema_migrations WHERE name=$1',
          [name],
        )
      ).rows[0];
      if (row) {
        if (row.checksum !== checksum) throw Error('Applied migration changed: ' + name);
        continue;
      }
